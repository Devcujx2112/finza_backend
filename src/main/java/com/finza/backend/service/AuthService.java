...
public class AuthService {
    ...
    public void forgotPassword(String email) {
        if (email == null || email.trim().isEmpty()) {
            throw new AppException(StatusCode.EmailAndPhoneNumberNull, BaseMessage.EmailAndPhoneNumberNull);
        } else {
            String normalizedEmail = email.trim();
            Account account = accountRepository.findByEmailIgnoreCase(normalizedEmail).orElseThrow(
                    () -> new AppException(StatusCode.EmailNotExists, BaseMessage.ACCOUNT_NOT_FOUND)
            );
            if (account.getProvider() != null) {
                throw new AppException(StatusCode.EmailIsRegisterInSocial, BaseMessage.SOCIAL_ACCOUNT_NO_PASSWORD);
            }
            otpService.sendForgotPasswordOtp(account.getEmail());
        }
    }
